...
<table class="min-w-full">
    ...
    <thead class="border-b border-t border-gray-200 bg-[#F79BD3] text-xs leading-4 font-semibold uppercase tracking-wider text-left">
    <tr>
    ...
    <tr class="border-t border-slate-600 text-sm" wire:key="{{ $item->getKey() }}">
        @if ($this->hasBulkActions)
        <td class="pl-3">
            <span class="flex items-center justify-center">
              <x-lv-checkbox value="{{ $item->getKey() }}" wire:model="selected"></x-lv-checkbox>
            </span>
        </td>
        @endif
        ...
        <tr>
            <td colspan="6" rowspan="1" class="pl-4 pb-2">
                <?php
                    $ourRent = $item;
                ?>
                @include('rents.rentedunitstable', $ourRent)
            </td>
        </tr>
    </tr>
    @endforeach
