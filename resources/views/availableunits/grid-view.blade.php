...
<x-lv-layout>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 {{ $cols[$maxCols] }} gap-8 md:gap-8 p-6">
    @foreach ($items as $item)
      <div class="relative">
        @if ($this->hasBulkActions)
          <div class="absolute top-0 lef-0 p-2">
            <x-lv-checkbox wire:model="selected" value="{{ $item->getKey() }}"></x-lv-checkbox>
          </div>
        @endif
    ...
    @if(count($items)==0)
        <div class="flex justify-center items-center p-4">
            <h3>{{ __('availableunits.table_empty') }}</h3>
        </div>
        <br>
    @endif
